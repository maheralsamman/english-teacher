...
{/*       <div className="grid sm:grid-cols-2 justify-items-center md:grid-rows-2 gap-4 p-8 text-white text-sm md:text-base ">
      ...
      <div className="flex justify-center gap-4 h-full w-full flex-col sm:flex-row sm:flex-wrap content-center items-center  sm:items-unset">
        {locale === "en"
          ? Object.entries(content.en.inprogress).map(([key, value], i) => (
              <div key={i} className="w-52 h-64 p-4 flex-col md:flex-row border-2 rounded border-[#7849ef]">
                <p className="inline text-[#31ff00] p-0 text-xs md:text-lg">{key}:</p>
                <p className="inline text-white text-xs md:text-base p-1">{value}</p>
              </div>
            ))
